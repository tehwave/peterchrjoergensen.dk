---
title: "A Quick Dirty WebRTC Tutorial with Manual Signaling"
description: "A short WebRTC primer showing how two browsers can connect directly with manual offer and answer codes, inspired by my multiplayer browser experiment."
pubDate: 2026-05-07
author: "Peter Chr. Jørgensen"
tags: ["webrtc", "browser", "p2p", "games"]
---

WebRTC sounds bigger than it is.

That does not mean it is simple. The API has sharp edges, and the vocabulary can make a small prototype feel like network engineering cosplay. SDP. ICE. STUN. TURN. Candidates. Offers. Answers.

But the basic idea is surprisingly approachable: **two browsers exchange connection details, then talk directly.**
...
In my experiment, I used it for a tiny air-hockey game. The host simulates the puck and scores. The client sends paddle input. The host sends state snapshots back. The connection is direct between browsers, while the website itself is still just a static Astro page.
...
It owns the negotiation state, network candidates, encryption setup, and the channels that eventually carry your data or media.
...
That STUN server is not your game server. It does not relay gameplay. It helps the browser discover how it might be reachable from the outside, which matters because most users sit behind NAT.

For real production systems, you usually also need TURN for relay fallback. For a quick experiment, a public STUN server can be enough to learn the shape of the API.
...
That local description contains SDP. SDP is not friendly. Treat it as an opaque blob. You do not need to hand-edit it. You just need to move it to the other browser.
...
Browsers can discover network candidates over time. In a normal app, you often send candidates incrementally through your signaling server. That is called trickle ICE.
...
Codes can be pasted into the wrong field. Share sheets can wrap text. Some networks will not connect without relay support. Some browsers support compression APIs and some do not. Mobile tabs can get backgrounded. Users will close the page and expect magic recovery.
...
If I were building something serious, I would add a signaling server and TURN fallback early. If I were building a strange portfolio experiment, I would absolutely keep the manual handshake. The friction is part of the point. It makes the architecture visible.
