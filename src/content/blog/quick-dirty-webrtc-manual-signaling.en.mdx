...
WebRTC requires a signaling exchange before two peers can connect, but it does not require a particular signaling server or transport. The peers only need a way to exchange offers, answers, and ICE candidates.

This article uses manual copy and paste. One player creates an invite code. The other player pastes it, generates a reply code, and sends that back. Production services usually automate the same exchange through a signaling server using WebSockets, HTTP, or another transport.

Once both browsers have the information they need, the WebRTC data channel opens and the game starts.
...
<RelatedContent heading="Read next" links={[{ href: "/projects/browser-multiplayer/", title: "I built a browser multiplayer game without a server" }]} />
